...
###Read in RFID data 
#raw file in seperate workspace file on github due to large size
...
#####Adding candelaperm2 to SQM data ####################
...
Q1m_candela<-glm(log(candelaperm2)~moon.light*bunker, data=SQM) 
outQ1m_candela<-summary(Q1m_candela)
outQ1m_candela
outQ1m_candela$coefficients

full <- glm(log(candelaperm2)~ships+moon.light*bunker, data=SQM) #not normal without log
...
length(unique(SQMSS$NightStarting)) #456 inc febs
...
hist(MOVE_SQMSS$activity) 

# poisson glm
Q2m_candela<-glm(IN_activity~-1+HRordered+MONTH+candelaperm2, data=MOVE_SQMSS, family=poisson) 
outQ2m_candela<-summary(Q2m_candela)
outQ2m_candela$coefficients

check_overdispersion(Q2m_candela) #overdispersed

#negative binomial glm
Q2m_candela_nb <- MASS::glm.nb(IN_activity~-1+HRordered+MONTH+candelaperm2, data=MOVE_SQMSS)
...
testOutliers(simulationOutput, type="bootstrap")
testDispersion(simulationOutput)
testZeroInflation(simulationOutput) #significant

#zero inflation model
Q2m_candela_z <- zeroinfl(IN_activity~-1+HRordered+MONTH+candelaperm2, data=MOVE_SQMSS)
outQ2m_candela_z<-summary(Q2m_candela_z)
outQ2m_candela_z$coefficients
outQ2m_candela_z
...
#zero inflation model
Q2m_candela_e <- zeroinfl(entrance_activity~-1+HRordered+MONTH+candelaperm2, data=MOVE_SQMSSe)
...
Q3m<-glm(IN_activity~HRordered+MONTH+bunker*moon.light, data=Mcc, family=poisson)
outQ3m<-summary(Q3m)
outQ3m$coefficients

check_overdispersion(Q3m) #poisson is overdispersed
simulationOutput <- simulateResiduals(fittedModel = Q3m, plot = F)
testDispersion(simulationOutput)

#negative binomial glm in MASS
Q3m_nb<-glm.nb(IN_activity~-1+HRordered+MONTH+bunker*moon.light, data=Mcc)
...
nw <- newdat %>% group_by(bunker) %>% summarise(mean=mean(fit), meanSE=mean(se.fit))

newdat <- newdat %>%
  mutate(upr=fit+se.fit)%>%
  mutate(lwr=fit-se.fit)

#plot raw data with number of ships labelled
ggplot(data=Mcc, aes(x=moon.light, y=IN_activity, colour=bunker, label=ships))+
  #geom_smooth(data=newdat, aes(x=moon.light, y=fit, colour=bunker), method = "glm")+
  geom_point(alpha = .5)+
  geom_text(aes(label=ifelse(IN_activity>10,as.character(ships),'')),hjust=0,vjust=1)+
  ylab("N individuals entering the cave") +
  xlab("moonlight (prop. of moon illuminated*degrees elevation)") +
  theme(panel.background=element_rect(fill="white", colour="black"), 
...
full3 <-glm.nb(IN_activity~-1+HRordered+MONTH+ships+bunker*moon.light, data=Mcc)
...
#neg bin glm 
Q3m_e<-glm.nb(entrance_activity~-1+HRordered+MONTH+bunker*moon.light, data=Mcce)
outQ3m_e<-summary(Q3m_e)
outQ3m_e$coefficients
outQ3m_e

#neg bin glm with number of ships
Q3m_e_sh<-glm.nb(entrance_activity~-1+HRordered+MONTH+ships+bunker*moon.light, data=Mcce)
